    ...
    <el-scrollbar height="calc(100vh)">
      <div class="aside-flex" style="height: calc(100vh)">
        <div>
          ...
          <el-menu :default-active="activeIndex" class="el-menu-vertical" :default-openeds="[1]">
            <!--主菜单-->
            <el-menu-item @mouseleave="laveMenu" @mouseenter="changeMenu(item.id,index)" @click="changeDrawer(item)" v-for="(item,index) in menuData.list" :key="item.id" :index='index+""'>
              <icon-park v-if="theme=='themeLight'" :type="item.iconName" theme="outline" :size="18" :strokeWidth="3" />
              <icon-park v-else :type="item.iconName" theme="outline" :size="18" :strokeWidth="4" />
              <span>{{item.name}}</span>
            </el-menu-item>
          </el-menu>
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    console.log("menuData size: "+menuData.list.length);
    console.log("submenus size: "+submenus.list.length);
    //监听路由
    let route = useRoute();
...
.themeBlack .text {
  ...
  border-radius: 4px;
  cursor: pointer;
  transition: background-color var(--el-text-color-primary),
...
/*-----------------dark主题------------------------*/
.dark .themeBlack,
.dark .themeBlack #menubar,
.dark .themeBlack .el-menu-vertical,
.dark .themeLight,
.dark .themeLight #menubar,
.dark .themeLight .el-menu-vertical {
  background: #000000 !important;
}

.dark .themeBlack .el-menu-item:hover,
.dark .themeLight .el-menu-item:hover {
  background-color: #000000;
}

.dark .themeBlack .el-menu-item,
.dark .themeLight .el-menu-item {
  color: #dedede !important;
  opacity: 0.8;
}

.dark .themeBlack .el-menu-item.is-active,
.dark .themeLight .el-menu-item.is-active {
  background-color: #000000;
  color: #ff6700 !important;
}

.dark .themeBlack .text,
.dark .themeLight .text {
  text-decoration: none;
  color: #fff;
  font-size: 14px;
  padding: 8px 16px;
...
.dark .themeBlack .text:hover,
.dark .themeLight .text:hover {
  background-color: var(--el-bg-color);
}

.dark .themeBlack .item .active,
.dark .themeLight .item .active {
