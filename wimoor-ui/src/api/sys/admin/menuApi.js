...
export const menuApi = {
	...
	getMenuALL: function(callback) {
		...
		for (let i = 0; i < menulist.length; i++) {  // 一级
			...
			if (menu.children) {
				...
				for (let j = 0; j < menu.children.length; j++) {  // 二级
					...
					if (child.children) {
						...
						for (let k = 0; k < child.children.length; k++) {  // 三级
							let item = child.children[k];
							if (item.path && (item.platform === undefined || item.platform === plat )) {
								namegrouplist.push({
									"id": item.name,
